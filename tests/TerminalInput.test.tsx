...
describe('TerminalInput', () => {
  ...
  it('wires terminal.onData to sendInput on mount', () => {
    render(<TerminalInput sendInput={mockSend} connected={true} />);
    expect(mockTerminal.onData).toHaveBeenCalledTimes(1);
  });

  it('sendInput receives the exact byte string from onData (Enter = \\r)', () => {
    render(<TerminalInput sendInput={mockSend} connected={true} />);
    capturedOnData!('\r');
    expect(mockSend).toHaveBeenCalledWith('\r');
  });

  it('forwards arrow-key ANSI sequences to sendInput', () => {
    render(<TerminalInput sendInput={mockSend} connected={true} />);
  ...
  it('forwards Ctrl+C, Ctrl+D, Tab control sequences to sendInput', () => {
    render(<TerminalInput sendInput={mockSend} connected={true} />);
    capturedOnData!('\x03');
